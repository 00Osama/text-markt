// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a ar locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes
// ignore_for_file:unnecessary_string_interpolations, unnecessary_string_escapes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'ar';

  static String m0(seconds) => "إعادة الإرسال خلال ${seconds} ثانية";

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
    "Date": MessageLookupByLibrary.simpleMessage("التاريخ"),
    "Done": MessageLookupByLibrary.simpleMessage("تم"),
    "Enter4DigitsPIN": MessageLookupByLibrary.simpleMessage(
      "أدخل رقم سري مكون من 4 أرقام",
    ),
    "Event": MessageLookupByLibrary.simpleMessage("الحدث"),
    "EventHint": MessageLookupByLibrary.simpleMessage("الحدث"),
    "Note": MessageLookupByLibrary.simpleMessage("الملاحظة"),
    "Notes": MessageLookupByLibrary.simpleMessage("الملاحظات"),
    "Time": MessageLookupByLibrary.simpleMessage("الوقت"),
    "Title": MessageLookupByLibrary.simpleMessage("العنوان"),
    "TitleHint": MessageLookupByLibrary.simpleMessage("العنوان"),
    "addToFavorites": MessageLookupByLibrary.simpleMessage("إضافة إلى المفضلة"),
    "addToHidden": MessageLookupByLibrary.simpleMessage("إخفاء"),
    "allNotes": MessageLookupByLibrary.simpleMessage("كل الملاحظات"),
    "alreadyHaveAnAccount": MessageLookupByLibrary.simpleMessage(
      "هل لديك حساب بالفعل ؟! ",
    ),
    "arabic": MessageLookupByLibrary.simpleMessage("العربية"),
    "areYouSureYouWantToSignOut": MessageLookupByLibrary.simpleMessage(
      "هل أنت متأكد أنك تريد تسجيل الخروج ؟!",
    ),
    "back": MessageLookupByLibrary.simpleMessage("رجوع"),
    "cancel": MessageLookupByLibrary.simpleMessage("إلغاء"),
    "checkYourEmail": MessageLookupByLibrary.simpleMessage(
      "تحقق من بريدك الإلكتروني",
    ),
    "clearQueryAndExit": MessageLookupByLibrary.simpleMessage(
      "مسح الاستعلام والخروج",
    ),
    "confirmPassword": MessageLookupByLibrary.simpleMessage(
      "تأكيد كلمة المرور",
    ),
    "darkMode": MessageLookupByLibrary.simpleMessage("الوضع الداكن"),
    "deleteEvent": MessageLookupByLibrary.simpleMessage("حذف الحدث"),
    "done": MessageLookupByLibrary.simpleMessage("مكتملة"),
    "dontHaveAnAccount": MessageLookupByLibrary.simpleMessage(
      "ليس لديك حساب ؟! ",
    ),
    "emailAddress": MessageLookupByLibrary.simpleMessage(
      "عنوان البريد الإلكتروني",
    ),
    "emailAlreadyInUse": MessageLookupByLibrary.simpleMessage(
      "هذا البريد الإلكتروني مستخدم بالفعل",
    ),
    "english": MessageLookupByLibrary.simpleMessage("الإنجليزية"),
    "enterAtLeastNoteTitle": MessageLookupByLibrary.simpleMessage(
      "أدخل عنوان الملاحظة على الأقل",
    ),
    "error": MessageLookupByLibrary.simpleMessage("خطأ"),
    "errorTitle": MessageLookupByLibrary.simpleMessage("خطأ"),
    "eventAddedSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تمت إضافة الحدث بنجاح",
    ),
    "eventDeletedSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تم حذف الحدث بنجاح",
    ),
    "eventExpired": MessageLookupByLibrary.simpleMessage("انتهى موعد الحدث"),
    "eventPreview": MessageLookupByLibrary.simpleMessage("معاينة الحدث"),
    "eventsTitle": MessageLookupByLibrary.simpleMessage("الأحداث"),
    "failedToAddEvent": MessageLookupByLibrary.simpleMessage(
      "فشل في إضافة الحدث",
    ),
    "failedToAddNote": MessageLookupByLibrary.simpleMessage(
      "فشل في إضافة الملاحظة",
    ),
    "failedToDeleteNote": MessageLookupByLibrary.simpleMessage(
      "فشل في حذف الملاحظة",
    ),
    "failedToUpdateNote": MessageLookupByLibrary.simpleMessage(
      "فشل في تحديث الملاحظة",
    ),
    "favorites": MessageLookupByLibrary.simpleMessage("المفضلة"),
    "favourite": MessageLookupByLibrary.simpleMessage("مفضله"),
    "fieldRequired": MessageLookupByLibrary.simpleMessage("هذا الحقل مطلوب"),
    "fillAllFields": MessageLookupByLibrary.simpleMessage(
      "يرجى ملء جميع الحقول قبل التحميل.",
    ),
    "forgotPassword": MessageLookupByLibrary.simpleMessage(
      "هل نسيت كلمة المرور ؟",
    ),
    "french": MessageLookupByLibrary.simpleMessage("الفرنسية"),
    "fullName": MessageLookupByLibrary.simpleMessage("الاسم الكامل"),
    "getStarted": MessageLookupByLibrary.simpleMessage("البدء"),
    "hidden": MessageLookupByLibrary.simpleMessage("المخفية"),
    "hiddenNotesPin": MessageLookupByLibrary.simpleMessage("ملاحظات مخفية"),
    "hide": MessageLookupByLibrary.simpleMessage("إخفاء"),
    "inProgress": MessageLookupByLibrary.simpleMessage("قيد التنفيذ"),
    "incorrectPin": MessageLookupByLibrary.simpleMessage(
      "الرقم السري غير صحيح",
    ),
    "invalidEmail": MessageLookupByLibrary.simpleMessage(
      "البريد الإلكتروني غير صحيح",
    ),
    "invalidSignin": MessageLookupByLibrary.simpleMessage(
      "بيانات تسجيل الدخول غير صحيحة",
    ),
    "invalidSignup": MessageLookupByLibrary.simpleMessage(
      "بيانات التسجيل غير صحيحة",
    ),
    "language": MessageLookupByLibrary.simpleMessage("اللغة"),
    "lightMode": MessageLookupByLibrary.simpleMessage("الوضع الفاتح"),
    "moveToTrash": MessageLookupByLibrary.simpleMessage("نقل إلى المحذوفات"),
    "newEvent": MessageLookupByLibrary.simpleMessage("حدث جديد"),
    "newNote": MessageLookupByLibrary.simpleMessage("ملاحظة جديدة"),
    "next": MessageLookupByLibrary.simpleMessage("التالي"),
    "noEvents": MessageLookupByLibrary.simpleMessage("لا توجد أحداث"),
    "noInternetConnection": MessageLookupByLibrary.simpleMessage(
      "لا يوجد اتصال بالإنترنت",
    ),
    "noNotes": MessageLookupByLibrary.simpleMessage("لا توجد ملاحظات"),
    "noPinSet": MessageLookupByLibrary.simpleMessage("لم يتم تعيين رقم سري"),
    "noRecentSearches": MessageLookupByLibrary.simpleMessage(
      "لا توجد عمليات بحث حديثة.",
    ),
    "noteAddedSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تمت إضافة الملاحظة بنجاح",
    ),
    "noteAddedToFavorites": MessageLookupByLibrary.simpleMessage(
      "تمت إضافة الملاحظة إلى المفضلة بنجاح",
    ),
    "noteAddedToHidden": MessageLookupByLibrary.simpleMessage(
      "تمت إضافة الملاحظة إلى المخفية بنجاح",
    ),
    "noteAddedToTrash": MessageLookupByLibrary.simpleMessage(
      "تمت إضافة الملاحظة إلى المحذوفات بنجاح",
    ),
    "noteDeletedSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تم حذف الملاحظة بنجاح",
    ),
    "noteSuccessfullyMovedTo": MessageLookupByLibrary.simpleMessage(
      "تم نقل الملاحظة بنجاح إلى",
    ),
    "noteUpdatedSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تم تحديث الملاحظة بنجاح",
    ),
    "notesTitle": MessageLookupByLibrary.simpleMessage("الملاحظات"),
    "ok": MessageLookupByLibrary.simpleMessage("حسنًا"),
    "onboarding1Description": MessageLookupByLibrary.simpleMessage(
      "طريقة سهلة بالكامل لإدارة وتخصيص ملاحظاتك.",
    ),
    "onboarding1TitleLine1": MessageLookupByLibrary.simpleMessage("نظم"),
    "onboarding1TitleLine2": MessageLookupByLibrary.simpleMessage(
      "ملاحظاتك بسهولة",
    ),
    "onboarding2Description": MessageLookupByLibrary.simpleMessage(
      "أجمل تطبيق لتدوين الملاحظات.",
    ),
    "onboarding2TitleLine1": MessageLookupByLibrary.simpleMessage("رتب"),
    "onboarding2TitleLine2": MessageLookupByLibrary.simpleMessage("أفكارك"),
    "onboarding3Description": MessageLookupByLibrary.simpleMessage(
      "جعل المحتوى واضحًا وسهل القراءة لم يكن أسهل من قبل.",
    ),
    "onboarding3TitleLine1": MessageLookupByLibrary.simpleMessage(
      "أنشئ بطاقات مع",
    ),
    "onboarding3TitleLine2": MessageLookupByLibrary.simpleMessage("تنظيم بسيط"),
    "password": MessageLookupByLibrary.simpleMessage("كلمة المرور"),
    "passwordMinLength": MessageLookupByLibrary.simpleMessage(
      "كلمة المرور يجب أن تكون 7 أحرف على الأقل",
    ),
    "passwordsNotMatch": MessageLookupByLibrary.simpleMessage(
      "كلمتا المرور غير متطابقتين",
    ),
    "pending": MessageLookupByLibrary.simpleMessage("قيد الانتظار"),
    "permanentlyDelete": MessageLookupByLibrary.simpleMessage("حذف نهائيا"),
    "pin": MessageLookupByLibrary.simpleMessage("أدخل الرقم السري"),
    "pinError": MessageLookupByLibrary.simpleMessage(
      "فشل في تعيين الرقم السري",
    ),
    "pinMustBe": MessageLookupByLibrary.simpleMessage(
      "الرقم السري يجب أن يكون مكون من 4 أرقام",
    ),
    "pinSetup": MessageLookupByLibrary.simpleMessage("إعداد رمز PIN"),
    "pinSuccessfullySet": MessageLookupByLibrary.simpleMessage(
      "تم تعيين الرقم السري بنجاح",
    ),
    "preferences": MessageLookupByLibrary.simpleMessage("التفضيلات"),
    "profile": MessageLookupByLibrary.simpleMessage("الملف الشخصي"),
    "profileTitle": MessageLookupByLibrary.simpleMessage("الملف الشخصي"),
    "recentSearches": MessageLookupByLibrary.simpleMessage("البحث الأخير"),
    "refreshCompleted": MessageLookupByLibrary.simpleMessage(
      "تم التحديث بنجاح",
    ),
    "resendEmail": MessageLookupByLibrary.simpleMessage("إعادة إرسال البريد"),
    "resendInSeconds": m0,
    "resetEmailSentSuccessfully": MessageLookupByLibrary.simpleMessage(
      "تم إرسال رسالة إعادة تعيين كلمة المرور بنجاح",
    ),
    "resetLinkSent": MessageLookupByLibrary.simpleMessage(
      "تم إرسال رابط إعادة تعيين كلمة المرور، تحقق من بريدك الإلكتروني.",
    ),
    "resetPasswordHint": MessageLookupByLibrary.simpleMessage(
      "سنرسل لك رابط إعادة تعيين كلمة المرور.",
    ),
    "resetPasswordTitle": MessageLookupByLibrary.simpleMessage(
      "إعادة تعيين كلمة المرور",
    ),
    "saveQueryToSearchHistory": MessageLookupByLibrary.simpleMessage(
      "حفظ الاستعلام في سجل البحث",
    ),
    "searchTitle": MessageLookupByLibrary.simpleMessage("بحث"),
    "sendLink": MessageLookupByLibrary.simpleMessage("إرسال الرابط"),
    "setPin": MessageLookupByLibrary.simpleMessage("تعيين الرقم السري"),
    "settings": MessageLookupByLibrary.simpleMessage("الإعدادات"),
    "signIn": MessageLookupByLibrary.simpleMessage("تسجيل الدخول"),
    "signOut": MessageLookupByLibrary.simpleMessage("تسجيل الخروج"),
    "signOutAndResetPassword": MessageLookupByLibrary.simpleMessage(
      "يرجي تسجيل الخروج وإعادة تعيين كلمة المرور",
    ),
    "signUp": MessageLookupByLibrary.simpleMessage("إنشاء حساب"),
    "skip": MessageLookupByLibrary.simpleMessage("تخطي"),
    "somethingWentWrong": MessageLookupByLibrary.simpleMessage(
      "حدث خطأ ما، قم بالتحديث لإعادة التحميل",
    ),
    "successTitle": MessageLookupByLibrary.simpleMessage("تم بنجاح"),
    "swipeForMoreOptions": MessageLookupByLibrary.simpleMessage(
      "اسحب لمزيد من الخيارات",
    ),
    "systemMode": MessageLookupByLibrary.simpleMessage("النظام"),
    "themeMode": MessageLookupByLibrary.simpleMessage("السمة"),
    "title": MessageLookupByLibrary.simpleMessage("العنوان"),
    "trash": MessageLookupByLibrary.simpleMessage("المحذوفات"),
    "tryAgain": MessageLookupByLibrary.simpleMessage("حاول مرة أخرى"),
    "tryAgainLater": MessageLookupByLibrary.simpleMessage(
      "حاول مرة أخرى لاحقًا",
    ),
    "typeYourEmail": MessageLookupByLibrary.simpleMessage(
      "اكتب عنوان بريدك الإلكتروني",
    ),
    "updateNote": MessageLookupByLibrary.simpleMessage("تحديث الملاحظة"),
    "userNotFound": MessageLookupByLibrary.simpleMessage("المستخدم غير موجود"),
    "usernameMinLength": MessageLookupByLibrary.simpleMessage(
      "اسم المستخدم يجب أن يكون 4 أحرف على الأقل",
    ),
    "validSignin": MessageLookupByLibrary.simpleMessage("تسجيل دخول صحيح"),
    "validSignup": MessageLookupByLibrary.simpleMessage("تسجيل حساب صحيح"),
    "verificationHint": MessageLookupByLibrary.simpleMessage(
      "لقد أرسلنا رابط تأكيد لتفعيل حسابك.",
    ),
    "verifyEmailTitle": MessageLookupByLibrary.simpleMessage(
      "تأكيد البريد الإلكتروني",
    ),
    "wrongPassword": MessageLookupByLibrary.simpleMessage(
      "كلمة المرور غير صحيحة",
    ),
    "wrongUsernameOrPassword": MessageLookupByLibrary.simpleMessage(
      "اسم المستخدم أو كلمة المرور غير صحيحة",
    ),
  };
}
